...
using Issue702.Infrastructure.Swagger;
...
namespace Issue702.Infrastructure.Configuration
{
              ...
              IConfigureOptions<SwaggerUIOptions>
    {
        ...
        {
            options.RouteTemplate = "issue702/swagger/{documentName}/swagger.json";
        }

        public void Configure(SwaggerUIOptions options)
        {
            options.RoutePrefix = "issue702/explorer";
            options.ConfigObject.DisplayRequestDuration = true;

            foreach (var description in Provider.ApiVersionDescriptions)
            {
                options.SwaggerEndpoint(
                    $"/issue702/swagger/{description.GroupName}/swagger.json",
                    $"Issue 702 {description.GroupName}");
            }
        }
    }
}
